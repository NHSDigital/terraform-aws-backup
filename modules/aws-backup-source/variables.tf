...
variable "backup_plan_config_efs" {
  description = "Configuration for backup plans with EBS"
  type = object({
    enable              = bool
    selection_tag       = string
  ...
  default = {
    enable                    = true
    selection_tag             = "BackupEFS"
    compliance_resource_types = ["EFS"]
    rules = [
      {
        name     = "efs_daily_kept_5_weeks"
        schedule = "cron(0 0 * * ? *)"
        lifecycle = {
          delete_after = 35
        }
